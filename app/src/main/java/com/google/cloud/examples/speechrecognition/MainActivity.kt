...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mTTS.speak("hello, how are you today", TextToSpeech.QUEUE_FLUSH, null)
                            ...
                            runOnUiThread {
                                when {
                                    value.resultsCount > 0 -> {
                                        val text = value.getResults(0).getAlternatives(0).transcript
                                        Service.sendPost(text, Consumer { mTextView.setText(it) })
                                    }
    ...
    fun post(input: String){
        Log.i(TAG, "Posting something")
    }
